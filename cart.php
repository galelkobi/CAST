			    ...
			    <?php if(isset($_SESSION['login_id'])){?>
			        <a href="homepage.php"><span class="fa fa-home mr-3"></span> Home</a>
			         ...
			         <?php if($_SESSION['typee'] == 'director'){?>
    				    ...
    				    <a class="active" href="cart.php"><span class="fa fa-shopping-cart mr-3"></span> Cart</a>
    				<?php }else{?>
...
<main id="clients" class="category-clients">
    <div class="">
		...
		<?php
			$i = 1;
			$qry0 = $conn->query("SELECT IFNULL(COUNT(*),0) num FROM users e LEFT JOIN cart c ON e.id = c.user_id WHERE c.id='" . $_SESSION['login_id'] . "'");
			while($row= $qry0->fetch_assoc()):
				$rownum = $row['num'];
			endwhile;
			$qry = $conn->query("SELECT e.id,e.lname,e.fname,MAX(p.photo) photo FROM users e LEFT JOIN cart c ON e.id = c.user_id LEFT JOIN users_photo p ON e.id = p.id_user WHERE c.id='" . $_SESSION['login_id'] . "' GROUP BY e.id,e.lname,e.fname");
										...
										<div class="center-vert">
											<div class="center-vert-inside">
												...
												<div class="add-to-cast-wrapper">
													<a href="#" id="<?php echo $row['id'] ?>" onclick = "remove_chart(<?php echo $row['id'] ?>)">
														<div class="text2">REMOVE FROM CAST</div>                                         
													</a>
												</div>
											</div>
										</div>
	...
	<script>
		...
		function remove_chart(id){
			console.log(id);
			$.ajax({
				url:'ajax.php?action=remove_chart',
				data: {id_user: id},
				method: 'POST',
				type: 'POST',
				success:function(resp){
					if(resp == 1){
						alert('Actor removed successfully to your chart.');
						setTimeout(function(){
